...
%% extract ACC channels and save as motion data

Motion = pop_select(EEG, 'channel', [EEG.nbchan-2 : EEG.nbchan]);
pop_saveset(Motion, 'filename',[ bemobil_config.filename_prefix num2str(subject) '_' bemobil_config.merged_motion_filename],'filepath',input_filepath);

EEG.chanlocs(end-2:end) = [];
EEG.data(end-2:end,:) = [];
...
chanloc_newname = {'Fp1', 'Fp2', 'F7', 'F3', 'Fz', 'F4', ...
    'F8', 'FC5', 'FC1', 'FC2', 'FC6', 'T7', 'C3', 'Cz', 'C4', 'T8', 'TP9', ...
    'CP5', 'CP1', 'CP2', 'CP6', 'TP10', 'P7', 'P3', 'Pz', 'P4', 'P8', ...
    'PO9', 'O1', 'Oz', 'O2', 'PO10'}; 
for n = 1:EEG.nbchan
    EEG.chanlocs(n).labels = chanloc_newname{n};
end
EEG = eeg_checkset(EEG);

pop_saveset(EEG, 'filename',[ bemobil_config.filename_prefix num2str(subject) '_' bemobil_config.merged_filename],'filepath',input_filepath);
